...
当然，compiler optimization 是个非常大的研究领域，包罗万象而且包含了各种 fancy tricks；所以本章节只整理了 LLVM 所有的 transformation passes 里需要的知识，并且整理在了下面。
如果系统学习这一块知识的需求的话，可以看一下这个文档 [#ref1]_，写的超级棒。
如果你真的只想看某一个特定的 pass，也可以直接移步 :doc:`LLVM-transformation-passes/index` 里对应的位置，那里写了那些 basics 是必须的。这样的回溯学习应该也能提升一些效率。
